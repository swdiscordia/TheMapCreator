...
using Ploup.Managers.Scene;
using Ploup.Models.Maps;
using UnityEngine;
using UnityEngine.AddressableAssets;

namespace Ploup.Models.Actors
{
    public class ActorSprite
    {
        ...
        public async UniTask MoveTo(short cell, Direction orientation, float speed)
        {
            ...
            var tween = GameObject.transform.DOMove(nextPosition, speed).SetEase(Ease.Linear);
            _moveTask = UniTask.WaitUntil(() => tween.IsComplete(), cancellationToken: _moveCancellationTokenSource.Token);
